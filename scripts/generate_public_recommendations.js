...
function buildOptionsPromptForStrategy(strategy, snapshot, tickerListStr) {
...
// Abschlusstest, Reviewer-Formulierung) — NICHT jetzt bearbeiten, s.u. ═══
...
// 2026, master_market_data.json vom selben Tag). ABER: die KI-Erklaerung im
// recommendation_text verwendet bislang ueberwiegend generische Faktoren
// (RSI, Grade, D200, IVP) und macht die ATMNA-spezifischen Ranking-Treiber
// bbPos/tightnessPct (genau die beiden Felder oben, die atmna von den
// anderen vier unterscheiden) in der Begruendung NICHT explizit sichtbar —
// liest sich stellenweise wie eine wiederverwendete CSP-Begruendung auf
// anderen Kandidaten, nicht wie eine genuin ATMNA-spezifische Erklaerung.
// KEIN Score-/Pipeline-Fehler, KEIN sofortiger Prompt-Fix (Axel-Entscheidung,
// 16.09.2026: Architektur/Datenfluss zuerst stabilisieren, Prompt-Qualitaet
// getrennt und gezielt angehen, nicht beides gleichzeitig). Bei der naechsten
// gezielten Options-Prompt-Qualitaetsrunde (in ko-prompts.js, nicht hier)
// pruefen, ob der atmna-Prompt bbPos/tightnessPct explizit als eigene
// Ranking-Begruendung einfordert, statt sie nur als Rohdaten im Ticker-
// Kontext mitzuliefern und dem Modell die Gewichtung zu ueberlassen.
...
async function runStrategy(strategy, masterData, snapshot, promptVersion) {
  try {
    ...
    const prompt = isOptions
      ? buildOptionsPromptForStrategy(strategy, snapshot, tickerListStr)
