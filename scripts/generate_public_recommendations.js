...
async function runStrategy(strategy, masterData, snapshot, promptVersion) {
  try {
    ...
    if (isOptions && typeof KoPrompts.validateBriefingCompliance === 'function') {
      const complianceCheck = KoPrompts.validateBriefingCompliance(strategy, apiResult.text);
      if (complianceCheck.status === 'FAIL') {
        console.warn(`  ⚠ ${strategy}: Pflichtangaben fehlen (${complianceCheck.missing.join(', ')}) — Repair-Versuch...`);
        try {
          const repairPrompt = buildRepairPrompt(strategy, complianceCheck.missing, apiResult.text);
          ...
          if (repairResult.ok) {
            const repairCheck = KoPrompts.validateBriefingCompliance(strategy, repairResult.text);
