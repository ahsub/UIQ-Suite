...
function buildPromptForStrategy(strategy, snapshot, tickerListStr, top10Count) {
  ...
  const ctx = { marktkontext, vixStr, optsCfg: DEFAULT_OPTS_CFG, isEic: false, mode: 'public', skipAbschnitt78: true };
  const result = KoPrompts.get(strategy, ctx);
  if (!result) {
    throw new Error(`KoPrompts.get("${strategy}", ctx) lieferte null — unbekannte Strategie-ID?`);
  }
  return result; // { prompt, abschnitt7, abschnitt8 }
}
...
async function finalizeStrategyResult(req, apiResult, snapshot, promptVersion) {
  ...
  try {
    ...
    // NEU (v1.22, 23.09.2026, Abschnitt-7+8-Templating): an dieser Stelle
    // ist apiResult.text garantiert final (REPAIR-FAILED/REPAIR-ERROR sind
    // oben bereits per return abgefangen) — validateBriefingCompliance()
    // lief bewusst VORHER auf dem rohen 1-6+9-Modelltext (unveraendert,
    // Abschnitt 7/8 spielen dort ohnehin keine Rolle, s. ko-prompts.js
    // v2.54.4-Changelog). Jetzt Abschnitt 7+8 deterministisch einfuegen,
    // BEVOR der Text in buildAiOutput() einfliesst und damit archiviert/
    // veroeffentlicht wird.
    try {
      const textWithAbschnitt78 = insertAbschnitt78(apiResult.text, req.abschnitt7, req.abschnitt8);
      const structCheck = validateAbschnitt78Structure(textWithAbschnitt78);
      if (!structCheck.ok) {
        return {
          ok: false, strategy,
          error: `Abschnitt-7/8-Struktur nach Einfuegen fehlerhaft: ${structCheck.problems.join('; ')}`,
        };
      }
      apiResult = { ...apiResult, text: textWithAbschnitt78 };
    } catch (splitErr) {
      return { ok: false, strategy, error: `insertAbschnitt78 fehlgeschlagen: ${splitErr.message}` };
    }
