...
13. **Präsentations-Feedback 11.07.2026** *(Live-Vorstellung, kleines interessiertes Publikum — Konzept insgesamt sehr positiv aufgenommen, als "innovativ" bezeichnet)* — fünf konkrete Rückmeldungen, alle noch unbewertet/nicht gebaut:
    ...
    - **(b) ✅ ERLEDIGT, ANDERS ALS URSPRÜNGLICH BESCHRIEBEN (verifiziert 24.08.2026) — Fehlende Strategiemodule ausarbeiten:** Dividende und Value existieren als Ampel-Gates (Farblogik in `getStrategyGates()`), aber es fehlt die dahinterliegende **Scan-Logik** (kein `score_long_dividend()`/`score_long_value()` in `market_aggregator.py`, analog zu `score_long_minervini()`/`score_long_swing()`). Ampel zeigt aktuell nur eine Regime-Einschätzung ohne echte Ticker-Auswahl dahinter für diese zwei Strategien. **Chronologie:** `score_long_dividend()`/`score_long_value()` wurden am 28.-29.07.2026 gebaut (Alpha-Desk-Leaderboard-Tabs 💰/📊, Backlog #13b). Parallel dazu — bereits am 17.07.2026, also VOR dem Scan-Logik-Bau — wurde bewusst entschieden, Dividend/Value komplett aus den Ampel-Gates zu entfernen (Regime-Coverage-Analyse: keine der beiden Strategien hat in irgendeinem der 5 MSE-Regime Priorität-1-Status, weil beide Buy-and-Hold-Konzepte sind, keine Timing-Strategien — die Ampel ist aber explizit ein Timing-Werkzeug). Ergebnis: die Scan-Logik existiert real und zeigt echte Ticker, aber im Leaderboard, nicht hinter der Ampel — architektonisch die richtige Lösung gegeben die 17.07-Entscheidung, keine halbe Umsetzung.
    - **(c) ✅ ERLEDIGT (25.07.2026, verifiziert 24.08.2026) — Ein-/Ausstiegskurs-Empfehlungen für Kurzzeit-Strategien:** Zwei Modi gewünscht — **User-Modus**: ATR-Bereichsangabe (relativ); **EIC-Modus**: konkrete EUR/USD-Kursangaben (absolut). Direkte Überschneidung mit der bereits portierten, aber noch nicht eingehängten Pattern/Entry-Engine (`ios_pattern_entry_engine.py`, 10.07.2026) — die liefert bereits `entry.suggested/buyStop/limitPullback/normalEntry/deepPullback/maxBuy` als Preispunkte; ATR-Bereichslogik wäre eine sinnvolle Ergänzung beim Einhängen. Stärkt das Argument für eine baldige Integrations-Entscheidung (siehe Roadmap Phase 1, Praesentation 11.07.). **Umgesetzt (Backlog #13c/f, 25.07.2026):** vollständig eingehängt — server- und clientseitig. `process_ticker()` ruft `score_pattern_setup()`/`score_entry_timing()` auf, Ergebnis im `patternEntry`-Feld je Ticker. Frontend: eigener farbcodierter Render-Block `#dd-pattern-entry` im DeepDive (Pattern-Entscheidung + Score, Entry-Grade + alle Preis-Punkte BuyStop/Suggested/LimPullback/MaxBuy), zusätzlich in den KI-Prompt-Kontext eingespeist. 24.08.2026 im Live-Code bestätigt, kein weiterer Handlungsbedarf.
    ...
    - **(e) Makro-Tab: ETF/Sektor-Klick zeigt Top-10-15-Holdings, optional als Watchlist ins Scanner-Tab übernehmbar. ✅ ERLEDIGT (11.07.2026, gleicher Abend).** Proof-of-Concept mit XLK vollständig gebaut und verifiziert: `parse_ssga_holdings_xlsx()` + `resolve_company_name_to_ticker()` (Yahoo Search, serverseitig, kein CORS-Proxy nötig) im Aggregator (`market.sectorHoldings.XLK`), Klick auf Sektor-RS-Zeile im Frontend zeigt Modal mit Top-15 + "In Watchlist übernehmen"-Button (`index.html` v301). 14/15 Ticker sofort aufgelöst, 15/15 nach Fallback-Fix (Aktienklassen-Zusätze wie "Class A" verwirrten die Suche). **Wichtige Einschränkung:** Quelle ist der State-Street-**UCITS**-Wrapper (europäische Notierung ZPDT GY), nicht der US-XLK direkt — Konstituenten nahezu identisch, Gewichtung kann wegen UCITS-Diversifikationsregeln leicht abweichen, im UI transparent ausgewiesen. **✅ VOLLSTÄNDIG ERLEDIGT (19.07.2026, verifiziert 24.08.2026) — alle 10 Sektor-ETFs:** `SECTOR_ETF_LIST = [XLK, XLY, XLF, XLE, XLV, XLI, XLU, XLP, XLC, XLB]`, jeweils eigene `data/holdings_{ETF}.xlsx` (EMEA-UCITS-Format), generisch über `build_sector_holdings()` geladen, im Live-Snapshot 24.08.2026 mit 147-150/150 Holdings korrekt aufgelöst (nach `_norm()`-Fix desselben Tages, s. Sektor-Holdings-Match-Rate-Verbesserung). **Offener Nachfolgepunkt (24.08.2026, s. Backlog №56):** alle 10 Holdings-Dateien stammen noch vom 19.07.2026 — 36 Tage alt trotz dokumentierter monatlicher Update-Pflicht (analog `iwv_holdings.csv`). Kein Blocker, aber realer Wartungsrückstand ohne Erinnerungsmechanismus.
    - **(f) ✅ ERLEDIGT gemeinsam mit (c) (25.07.2026, verifiziert 24.08.2026) — Call-/Put-Level-Bestimmung im Enrichment-Lauf (Aggregator + Scanner-Shortlists):** Wunsch, für Einzelaktien im Enrichment-Lauf bereits Call-/Put-Level zu visualisieren/bestimmen, um Realize-Profit- und Stop-Loss-Entscheidungen zu unterstützen. Direkte fachliche Überschneidung mit (c) — die bereits portierte Pattern/Entry-Engine (`ios_pattern_entry_engine.py`) liefert schon `entry.suggested/buyStop/limitPullback/normalEntry/deepPullback/maxBuy`. Zusätzlicher Aspekt hier: explizit auf Enrichment-/Shortlist-Ebene (nicht nur Einzelticker-Scan), also mehrere Ticker gleichzeitig im nächtlichen Lauf. Beide Punkte (c)+(f) sollten bei der Integrations-Entscheidung zur Pattern/Entry-Engine zusammen behandelt werden, nicht getrennt. **Wie unter (c) beschrieben zusammen umgesetzt** — `patternEntry` läuft für jeden gescannten Ticker im nächtlichen Enrichment mit, nicht nur bei Einzelticker-Abfragen, erfüllt damit auch den hier zusätzlich geforderten Shortlist-Aspekt.
...
32. ✅ **ERLEDIGT (24.08.2026) — Testgürtel Regime-Pfad** *(SWOT W1/W2/T3, 07.08.2026)* — Unit-Tests für: `_ratio_to_regime()`-Äquivalent, Regime-Klassifikation mit beiden Ratio-Konventionen (VIX/VIX3M und VIX3M/VIX), `calc_regime_history_flag()` (alle 5 Szenarien bereits manuell getestet → formalisieren). Ablage: `ko-aggregator/tests/test_regime.py`. Schützt das wertvollste Asset: jeder Scoring-Bug der rückwirkend entdeckt wird kontaminiert den Track-Record. **Verifikation 24.08.2026:** Testdatei existierte bereits (25 Tests, alle grün), aber Zeile-für-Zeile-Abgleich mit `classify_regime_v2()` deckte eine echte Lücke auf — der GEX<0-Override-Zweig war komplett ungetestet. 6 neue Tests ergänzt (Override greift bei BULL_QUIET/BULL_FRAGILE, greift NICHT bei POST_PANIC/STRESS_UNSTABLE). 31/31 Tests grün, committed & aus dem Live-Commit erneut verifiziert. Hash `571d527e6e0351f81de11a42c27b525b7e9e70d1`.
...
37. **OptionsCoach + OptionsDoktor — KI-gestützter Options-Coaching-Assistant** *(07.08.2026, aus Analyse Flex-XML-Datenbasis)*

   **Kernidee:** Die Kombination aus echten Trade-Daten (Flex-XML 2023–2026), Marktdaten zum Eröffnungszeitpunkt (UIQ: Regime, IV-Rank, RSI), steuerlicher Einordnung (Refundex) und KI-Analyse (Claude) ergibt ein Coaching-Tool das es so nirgendwo gibt.

   **Zwei Modi:**

   *OptionsCoach (prospektiv — vor dem Trade):*
   Prüft vor Eröffnung: Regime-Check (UIQ), Earnings-Gate, IV-Rank-Attraktivität,
   Delta-Auswahl je Regime, Positionsgröße. Verhindert bekannte Fehler systematisch.

   *OptionsDoktor (retrospektiv + laufend — nach/während dem Trade):*
   → Laufende Position: Delta gestiegen? Rollen vs. Rückkauf vs. Hedge vs. Halten.
     Konkrete Handlungsoptionen mit steuerlicher Konsequenz je Option.
   → Abgeschlossene Position: Diagnose — was lief schief?
     Vergleich mit alternativer Strategie. Lernpunkt extrahiert.
...
46. **VIX/VIX3M Term Structure (Contango/Backwardation)** *(09.08.2026, aus Pine-Script-Marktsichtung Options-Indikatoren)* — Prio 1, reale Index-Ticker (kein synthetischer Proxy).

    **Konzept:** Verhältnis VIX3M/VIX als struktureller Marktzustands-Indikator
    für Premium-Selling-Strategien (Wheel/CSP):
    - Ratio < 1.0 → Contango → struktureller Rückenwind für Premium-Selling
    - Ratio > 1.0 → Backwardation → Stress-Regime, Premium-Selling reduzieren
    - Ratio > 1.10 → Deep Backwardation, historisch beste Selling-Chance,
      aber erst nach Mean-Reversion-„Hook" abwarten (nicht sofort selling)

    **UIQ-Anschluss:** Passt konzeptionell zur bestehenden 4-Regime-Engine,
    insbesondere STRESS_UNSTABLE. Zwei Optionen, nicht vorentschieden:
    (a) zusätzliche Zeitreihe in Backlog №44 (Data Foundation), oder
    (b) eigenständiger DCE-Input. Datenquelle: reale VIX/VIX3M-Ticker,
    kein Pine-spezifischer Proxy nötig — direkt in Python nachbaubar.

    **Kein Bau vor:** Entscheidung (a) vs. (b), da das die Kalibrierungslogik
    (Brier Score, CODING-RULES §2.6 Regime-Singularität) unterschiedlich betrifft.

47. **Put/Call Ratio (CBOE CPC/PCC)** *(09.08.2026, aus Pine-Script-Marktsichtung Options-Indikatoren)* — Prio 1, reale CBOE-Daten (kein synthetischer Proxy).

    **Konzept:** Ergänzt den bestehenden FINRA Reg SHO DIX um eine zweite,
    unabhängige Sentiment-Dimension (Options-Markt statt Short-Interest).

    **UIQ-Anschluss:** Reale CBOE-Ticker (CPC/PCC), keine Pine-spezifische
    Beschränkung — direkt portierbar. Kandidat für Backlog №44 (Data Foundation)
    als zusätzliche Zeitreihe neben DIX, nicht als Ersatz.

    **Kein Bau vor:** Klärung, ob als eigenständige Metrik oder als
    DIX-Kombinationsindikator geführt (vermeidet Doppel-Zählung von Sentiment
    im Composite Score).

    *Verwandt mit: №44 (UIQ Data Foundation). Herkunft beider Punkte: adversarialer
    Pine-Script-Review (Bear Put Spread v1/PROv2, ChartPrime Bayesian Trend) +
    gezielte Marktsichtung TradingView-Bibliothek nach Options-Konzepten,
    09.08.2026. Details: `docs/UEBERGABE-2026-08-09-pine-review.md`. Architektur-Fund
    aus derselben Session: Pine Script hat keinen Zugriff auf Echtzeit-Optionsketten —
    GEX-/IV-Rank-Indikatoren in Pine sind entweder reine Visualisierung extern
    vorberechneter Werte oder synthetische Historik-Proxys ohne echten
...
49. **Optionsmodul — Kognitive Architektur v1.0** *(09.08.2026, Axel-Manifest + Claude-Ausarbeitung)* — Grundsatzdokument: UIQ ist kein Dashboard/Scanner/Indikator-Sammlung, sondern ein mehrstufiges Entscheidungsmodell nach diagnostischem Vorbild (Axels Praxis als Dermatologe als Strukturgeber).

    **6-Stufen-Kaskade:** Markt verstehen → Investor verstehen → Underlying
    verstehen (KI-assistiert) → Strategie auswählen (KI-assistiert) →
    Trade begleiten (KI-assistiert) → Aus Fehlern lernen (KI-assistiert).
    Verbindet UIQ (Stufen 1–4, Markt-/Underlying-Diagnose) mit Refundex
    (Stufen 5–6, Journal + OptionsDoktor, ROADMAP 2.9/2.12).

    **Inhalt:** Strategiekatalog A–E (Directional-Neutral, Directional,
    Volatility, Range-Bound, Volatility Spreads — je mit Erwartung,
    Chart-Kontext, Greeks-Optimalwerten); charttechnische Trigger
    (Support/Resistance >30 Tage, Breakout mit Volumenbestätigung,
    Earnings/News-Events); Volatilitäts-Kegel (IV-Perzentil-Logik,
    >90P verkaufen / <10P kaufen); 2-stufige Entscheidungsmatrix
    (Richtung × IV-Rang → Strategieempfehlung).

    **Entschieden (09.08.2026):**
    - Investoren-Profil: zweistufig — Onboarding-Selbsteinschätzung +
      Abgleich gegen reales Trading-Verhalten (Diskrepanz-Flag via
      OptionsDoktor-Lernmuster-Engine)
    - CapTrader/IBKR-Anbindung: beide Zwecke parallel — Live-Metriken
      (Greeks/Earnings/Optionsketten) primär für UIQ-Options-Scorer,
      Trade-Dokumentation primär für Refundex-Journal; eine Schnittstelle
      (IBKR TWS API), Flex Query bleibt separat für Steuer-Historie
    - Regime-Mapping v1 (BULL_QUIET→Covered Calls weiter OTM,
      BULL_FRAGILE→Collars/weniger Selling, POST_PANIC_REVERSION→
      aggressive Premium-Ernte, STRESS_UNSTABLE→defensiv/Long Vega bis
      Hook) — **Lehrbuch-Logik, nicht gegen Backtest validiert.**

    **Kein Bau vor:** Regime-Mapping-Validierung gegen 2007–2026-Backtest
    (Gate-A Sharpe 1,66-Infrastruktur bereits vorhanden); IBKR-TWS-API-
...
56. **Sektor-Holdings-Update-Erinnerung + Quellenreferenz** *(24.08.2026, aus
    ...
    (XLK/XLY/XLF/XLE/XLV/XLI/XLU/XLP/XLC/XLB) stammen vom 19.07.2026 — 36 Tage
    alt zum Zeitpunkt dieses Eintrags, trotz dokumentierter monatlicher
    Update-Pflicht (analog `iwv_holdings.csv`, zuletzt 20.08.2026
    aktualisiert). Bislang kein Erinnerungsmechanismus — die Staleness fällt
    nur auf, wenn jemand gezielt nachschaut (wie hier).

    **Quellen (verifiziert 24.08.2026, per Websuche, nicht aus dem Code
    geraten):** Die Holdings-Dateien sind EMEA-UCITS-Proxys der State-Street-
    SPDR-"S&P U.S. [Sektor] Select Sector UCITS ETF"-Fondsfamilie
    (`ssga.com`, UK/EMEA-Institutional-Seite). Bestätigte Ticker-Zuordnung:

    | US-ETF | UCITS-Proxy-Ticker | Fondsname |
    |---|---|---|
    | XLK | ZPDT | SPDR S&P U.S. Technology Select Sector UCITS ETF |
    | XLF | ZPDF | SPDR S&P U.S. Financials Select Sector UCITS ETF |
    | XLE | ZPDE | SPDR S&P U.S. Energy Select Sector UCITS ETF |
    | XLY, XLV, XLI, XLU, XLP, XLC, XLB | *(noch nicht einzeln verifiziert)* | vermutlich dieselbe Fondsfamilie, gleiches ZPD*-Namensschema |

    **Bewusst nicht geraten:** die restlichen 7 Ticker/Fondsnamen, um keine
    falschen ISINs/Ticker in ein Referenzdokument zu schreiben, das später
    für automatisierte Downloads genutzt werden könnte. Axel kennt den
    tatsächlichen Bezugsweg (er beschafft die Dateien manuell) — Ergänzung
    der Tabelle bei Gelegenheit sinnvoll, kein Blocker.

    **Erinnerungsfunktion — noch nicht gebaut, zwei Optionen zur Wahl:**
    (a) einfache Alters-Prüfung im Aggregator-Lauf selbst: Dateidatum von
    `data/holdings_{ETF}.xlsx` gegen 30 Tage prüfen, bei Überschreitung
    Log-Warnung + optional Eintrag im ohnehin neuen `degraded_status`-KV-Key
    (Backlog №35) als separates Feld `sectorHoldingsStaleAgeDays`, sodass es
...
59. **"Was wäre wenn"-Analyse** *(07.08.2026, aus Analyse Flex-XML-Datenbasis —
   ...
   **Datenfluss:**
   ...
   Flex-XML (alle Trades 2023–2026)
     → parseActivityXML() + Position-Aggregation (Rolls erkannt)
     → OptionsDoktor-Kontext: {position, eröffnung, rolls, aktuell, steuer}
     → Claude Analyse (Strict Extraction Mode, nur Fakten)
     → Diagnose + Handlungsoptionen + Lernpunkt
   ```

   **Alleinstellungsmerkmal:** Kein anderes Tool kombiniert:
   echte Trade-Daten + Marktbedingungen zum Entry + steuerliche Einordnung + KI-Coaching.
   TastyTrade kennt Trades, nicht das Regime. UIQ kennt das Regime, nicht die Trades.
   Refundex kennt beides, macht kein Coaching. OptionsDoktor verbindet alle drei.

   **Architektur:**
   - `ko-journal.js` (ROADMAP 2.9, heute implementiert) = Datenbasis
   - Position-Aggregation: Fills → logische Positionen (Roll = ein Ereignis, nicht 2 Trades)
   - Roll-Erkennung: gleicher Underlying + neuer Strike/Expiry direkt danach
   - UIQ-Integration: Regime + IV-Rank + RSI zum Entry-Zeitpunkt aus mse_history/iv_history
   - Claude-Schicht: Strict Extraction Mode, keine Empfehlungen ohne Datenbasis
   - Lernmuster-Engine: Aggregation über alle abgeschlossenen Positionen

   **Zuordnung:** Refundex (post-trade) oder eigenständiges Modul.
   Entscheidung offen bis UIQ Phase 1 (Track Record) ausgewertet.
   **Trigger:** nach 01.10.2026 (Track Record + IV-Rank reif, Datenbasis vollständig).
   Verwandt mit: Refundex ROADMAP 2.9 (ko-journal.js), UIQ Track Record, ML_KONZEPT DCE.
...
69. **Pareto-Roadmap „Beweisbarkeit vor Features" (neu 24.09.2026)** *(aus
    ...
    **Verbindliche Reihenfolge:**
    ...
    0. **Abschnitt-7/8-Templating abschließen** (Plan vom 13.09.2026) —
       **Stand ungeklärt, vor Beginn im Code verifizieren.** Zwei Aussagen
       vom 24.09.2026 widersprechen sich: (a) `UEBERGABE-2026-09-24.md`:
       Templating v1.22 → Produktions-Incident (Prompt-Anweisungen statt Text
       in Abschnitt 7+8 aller 15 Strategien), v1.23 = Rückbau, Neuaufbau als
       „Option B“ mit drei getrennten Rollen (`PromptInstruction78()` /
       `BuildPublicSection78()` / `insertAbschnitt78()`) festgelegt, aber
       nicht gebaut; (b) Recherche-Chat: „begonnen, Restpunkt
       `finalizeStrategyResult()`“. Wird vor A abgeschlossen, kein Umschwenken
       mitten im Umbau; die Roadmap für 25.09.2026 (Nachtlauf-Prüfung,
       Commit A Phase C, `fading_short`-Token-Limit) hat Vorrang vor №69.
    ...
    **Zurückgestellt (bewusst nicht jetzt):**
    ...
    - *Stufe C (Fair-Value-Auftauen bzw. v2.0):* Owner-Earnings-Kern —
      ...
      (= Canonical Metrics Pipeline / Option B); Uneinigkeit der
    ...
    **Nicht mehr separat:** automatische Ticker-Universum-Pflege — läuft
    bereits als TICKER_MASTER Phase C + `add_ticker.py`; der
    Survivorship-Hinweis betrifft den offenen ex-IWV-Befund.
