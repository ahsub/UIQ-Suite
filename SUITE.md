...
13. **Präsentations-Feedback 11.07.2026** *(Live-Vorstellung, kleines interessiertes Publikum — Konzept insgesamt sehr positiv aufgenommen, als "innovativ" bezeichnet)* — fünf konkrete Rückmeldungen, alle noch unbewertet/nicht gebaut:
    ...
    - **(e) Makro-Tab: ETF/Sektor-Klick zeigt Top-10-15-Holdings, optional als Watchlist ins Scanner-Tab übernehmbar.** Konkrete, gut abgegrenzte UX-Idee. Bräuchte eine Holdings-Datenquelle je ETF (ähnliches Muster wie das bestehende `iwv_holdings.csv` für das Scan-Universum, siehe monatliche Update-Erinnerung) — noch nicht geprüft ob eine kostenlose Quelle für beliebige Sektor-ETF-Holdings existiert oder ob das auf wenige Kern-ETFs beschränkt bleiben müsste.
    ...
    - **(g) Klarnamen der Ticker (mehrfach geäußert):** Bereits früher diskutiert und teilweise gelöst — die INPUT-Richtung (Klarname eintippen → Ticker auflösen) läuft schon über Yahoo Finance Search (kein hardcoded Mapping mehr, bewusste Entscheidung wegen Index-Umschichtungen). Der jetzige Wunsch zielt auf die **Anzeige-Richtung**: Ticker in Scan-Ergebnissen/Shortlists sollen den Klarnamen mit anzeigen (z.B. "AAPL — Apple Inc." statt nur "AAPL"), nicht nur bei der Eingabe auflösbar sein. Technisch vermutlich über dieselbe Yahoo-Search-API lösbar (Ticker→shortname-Reverse-Lookup + Caching), aber eigenständige Aufgabe von der Input-Auflösung — noch nicht umgesetzt, nur mehrfach als Wunsch genannt.
